# frozen_string_literal: true

module Api
  class ChecksController < ActionController::API
    protect_from_forgery with: :null_session if respond_to?(:protect_from_forgery)

    def create
      return head :accepted if ignore_event?

      payload = raw_payload
      repo = find_repo!(payload)
      check = repo.checks.create!(commit_id: commit_sha(payload))
      check.perform!

      render json: { id: check.id, state: check.state, exit: check.exit_status }, status: response_status
    rescue ActiveRecord::RecordNotFound
      render json: { error: 'repository_not_found' }, status: :not_found
    rescue StandardError => e
      render json: { error: e.message }, status: :unprocessable_entity
    end

    private

    def event_name
      @event_name ||= request.headers['X-GitHub-Event'].to_s
    end

    def push_event?
      event_name == 'push'
    end

    def ignore_event?
      event_name.present? && !push_event?
    end

    def response_status
      push_event? ? :created : :ok
    end

    def raw_payload
      params.to_unsafe_h
    end

    def repo_data(payload)
      payload['repository'] || {}
    end

    def commit_sha(payload)
      payload['after'] || payload.dig('head_commit', 'id')
    end

    def find_repo!(payload)
      data = repo_data(payload)
      github_id = data['id']
      full_name = data['full_name']

      if github_id.present?
        Repository.find_by!(github_id: github_id)
      elsif full_name.present?
        Repository.find_by!(full_name: full_name)
      else
        raise ActiveRecord::RecordNotFound
      end
    end
  end
end
